...
ros::Publisher pub_odometry, pub_latest_odometry, pub_virOdometry;
ros::Publisher pub_path, pub_relo_path, pub_virPath;
ros::Publisher pub_point_cloud, pub_margin_cloud;
ros::Publisher pub_key_poses;
ros::Publisher pub_relo_relative_pose;
ros::Publisher pub_camera_pose;
ros::Publisher pub_camera_pose_visual;
nav_msgs::Path path, relo_path, virPath;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_virPath = n.advertise<nav_msgs::Path>("virPath", 1000);
    pub_relo_path = n.advertise<nav_msgs::Path>("relocalization_path", 1000);
    pub_odometry = n.advertise<nav_msgs::Odometry>("odometry", 1000);
    pub_virOdometry = n.advertise<nav_msgs::Odometry>("virOdometry", 1000);
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        nav_msgs::Odometry virOdometry;
        virOdometry.header = header;
        virOdometry.header.frame_id = "world";
        virOdometry.child_frame_id = "world";
        Quaterniond tmp_Q_vir;
        tmp_Q_vir = Quaterniond(estimator.virOri);
        virOdometry.pose.pose.position.x = estimator.virPos.x();
        virOdometry.pose.pose.position.y = estimator.virPos.y();
        virOdometry.pose.pose.position.z = estimator.virPos.z();
        virOdometry.pose.pose.orientation.x = tmp_Q_vir.x();
        virOdometry.pose.pose.orientation.y = tmp_Q_vir.y();
        virOdometry.pose.pose.orientation.z = tmp_Q_vir.z();
        virOdometry.pose.pose.orientation.w = tmp_Q_vir.w();
        virOdometry.twist.twist.linear.x = estimator.virVel.x();
        virOdometry.twist.twist.linear.y = estimator.virVel.y();
        virOdometry.twist.twist.linear.z = estimator.virVel.z();

        pub_virOdometry.publish(virOdometry);

        geometry_msgs::PoseStamped vir_pose_stamped;
        vir_pose_stamped.header = header;
        vir_pose_stamped.header.frame_id = "world";
        vir_pose_stamped.pose = virOdometry.pose.pose;
        virPath.header = header;
        virPath.header.frame_id = "world";
        virPath.poses.push_back(vir_pose_stamped);
        pub_virPath.publish(virPath);
...
void pubKeyPoses(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    for (unsigned i = 0; i <estimator.PsLong_result.size(); i++)
    {
        geometry_msgs::Point pose_marker;
        pose_marker.x = estimator.PsLong_result.at(i).x();
        pose_marker.y = estimator.PsLong_result.at(i).y();
        pose_marker.z = estimator.PsLong_result.at(i).z();
        key_poses.points.push_back(pose_marker);

        pose_marker.x = estimator.PsLong.at(i).x();
        pose_marker.y = estimator.PsLong.at(i).y();
        pose_marker.z = estimator.PsLong.at(i).z();
        key_poses.points.push_back(pose_marker);
    }
