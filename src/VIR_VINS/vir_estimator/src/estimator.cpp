...
void Estimator::setParameter()
{
    ...
    if (KNOWN_ANCHOR == 1)
    {
        anchor_pos[0] = ANCHOR_POS[0];
        anchor_pos[1] = ANCHOR_POS[1];
        anchor_pos[2] = ANCHOR_POS[2];
    }
    else
    {
        anchor_pos[0] = 0;
    ...
    para_Pose_Long = new double*[WINDOW_SIZE_LONG];
    
    for(int i = 0; i < WINDOW_SIZE_LONG; ++i)
        para_Pose_Long[i]= (new double[SIZE_POSE]);    
...
void Estimator::vector2double()
{
    ...
    for (unsigned i = 0; i < PsLong.size(); i++)
    {
        para_Pose_Long[i][0] = PsLong.at(i).x();
        para_Pose_Long[i][1] = PsLong.at(i).y();
        para_Pose_Long[i][2] = PsLong.at(i).z();
        Quaterniond q{RsLong.at(i)};
...
void Estimator::double2vector()
{
    ...
    double y_diff = origin_R0.x() - origin_R00.x(); // Course the optimization is relative trajectory optimization, now change to the orig point.
    //TODO
    ...
    if (PsLong.size() == WINDOW_SIZE_LONG && solver_flag == NON_LINEAR)
    {
        //RsLong_result.clear();
        PsLong_result.clear();
        for (unsigned i = 0; i < PsLong.size(); i++)
        {
            ...
            PsLong_result.push_back( Vector3d(para_Pose_Long[i][0],
                            para_Pose_Long[i][1],
                            para_Pose_Long[i][2]));   
        ...
        if ((PsLong_result.back()-origin_P0).norm()<0.9 && (PsLong_result.back()-origin_P0).norm()>0.01)
        {
            //cout<<" ========== pose jump  ================ "<< endl;
            origin_P0 = PsLong_result.back()+Ps[0]-PsLong.back();

            //RsLong.clear();
            PsLong.clear();

            for (unsigned i = 0; i <WINDOW_SIZE_LONG; i++)
            {
                //RsLong_result.push_back(rot_diff * Quaterniond(para_Pose_Long[i][6], para_Pose_Long[i][3], para_Pose_Long[i][4], para_Pose_Long[i][5]).normalized().toRotationMatrix());
                
                PsLong.push_back( Vector3d(para_Pose_Long[i][0],
                                para_Pose_Long[i][1],
                                para_Pose_Long[i][2]));   
...
void Estimator::optimization()
{
    ...
    if(USE_UWB && PsLong.size() == (WINDOW_SIZE_LONG) && KNOWN_ANCHOR == 1)
    {  
...
Estimator::~Estimator() {
    for(int i = 0; i < WINDOW_SIZE_LONG; ++i) {
    delete [] para_Pose_Long[i];
}
delete [] para_Pose_Long;
